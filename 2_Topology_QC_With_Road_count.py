...
for root_dir, dirs, files in os.walk(input_folder):
    for file in files:
        if file.endswith("_polygons.shp"):
            ...
            intersect_fc = os.path.join(output_folder, village_full + "_INT.shp")

            for temp in [overlap_fc, intersect_fc]:
            ...
            # ---------------- GAP ----------------
            dissolve_fc = os.path.join(output_folder, village_full + "_DISSOLVE.shp")
            line_fc = os.path.join(output_folder, village_full + "_LINE.shp")
            poly_fc = os.path.join(output_folder, village_full + "_POLY.shp")
            gap_fc = os.path.join(output_folder, village_full + "_GAP.shp")

            for temp in [dissolve_fc, line_fc, poly_fc, gap_fc]:
                if arcpy.Exists(temp):
                    arcpy.Delete_management(temp)
            ...
            if road_fc and arcpy.Exists(road_fc):
                ...
                inter_fc = os.path.join("in_memory", "road_inter")

                if arcpy.Exists(inter_fc):
                    arcpy.Delete_management(inter_fc)
            ...
            # remove duplicates
            road_A = list(set(road_A))
            road_B = list(set(road_B))
            road_C = list(set(road_C))

            road_A_str = ",".join(road_A) if road_A else "-"
            road_B_str = ",".join(road_B) if road_B else "-"
            road_C_str = ",".join(road_C) if road_C else "-"
...
print("\n🔥 FINAL TOPOLOGY + ROAD QC DONE (NO ERROR):", csv_path)
